...
loop_max:
    ...
    lw      $t2, 0($t2)         # $t2 = V[i]
    slt     $t4, $t0, $t2       # $t4 = 1 if max < V[i]
    beq     $t4, $zero, next_max  # If max >= V[i], skip updating max
    # Update max and max_index
    add     $t0, $t2, $zero     # max = V[i]
    add     $t3, $t1, $zero     # max_index = i
next_max:
    addi    $t1, $t1, 1         # i++
    j       loop_max            # Loop back
done_max: 
    add     $v0, $t3, $zero     # return max_index
    jr      $ra
...
sloop:
    ...
    #starts here
    sll     $t0, $v0, 2            # $t0 = MaxIndex * 4 (byte offset)
    add     $t0, $t0, $a0          # $t0 = address of V[MaxIndex]

    # Calculate address of last element (n - 1)
    addi    $t1, $s0, -1           # $t1 = n - 1
    sll     $t1, $t1, 2            # $t1 = (n - 1) * 4 (byte offset)
    add     $t1, $t1, $a0          # $t1 = address of V[n - 1]

    # Swap V[MaxIndex] and V[n - 1]
    lw      $t2, 0($t0)            # Load V[MaxIndex] into $t2
    lw      $t3, 0($t1)            # Load V[n - 1] into $t3

    sw      $t2, 0($t1)            # Store V[MaxIndex] at V[n - 1]
    sw      $t3, 0($t0)            # Store V[n - 1] at V[MaxIndex]

    # Decrement the array size (n = n - 1)
    addi    $s0, $s0, -1           # Decrease the size of the unsorted array
    #ends here
